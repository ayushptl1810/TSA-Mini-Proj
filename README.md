...
When a variable is not observed at time _t_, instead of forward-filling, the
...
| **Missingness pattern** | Dead patients have _shorter_ observation gaps (13.2h vs 14.3h) — intensive monitoring = sicker patient. Missingness is MAR/MNAR, not random |
...
The project includes a high-fidelity Streamlit dashboard for real-time ICU monitoring and mortality risk inference.

### Key Features

- **Inference Engine**: Upload patient vitals in CSV format to run live predictive analysis using the trained Latent-ODE weights.
- **Dynamic 48h Preprocessing**: Automatically bins, normalizes, and computes temporal decay (delta) for uploaded clinical data.
- **Visual TSA Dashboard**: Interactive exploration of missingness patterns, autocorrelation (ACF/PACF), and observation density across 36 variables.
- **Survival Analytics**: Per-patient $S(t)$ trajectory tracking and multi-horizon early warning signals (6h horizon).
